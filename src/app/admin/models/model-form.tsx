...
const mediaCategories: MediaCategory[] = [
  ...
  {
    accept: "image/*",
    description: "Digitals e polaroids para avaliação rápida.",
    emptyLabel: "Ainda sem materiais cadastrados",
    id: "polaroids",
    mediaType: "polaroid",
    placeholder: "Imagem",
  ...
  {
    description: "Cartão visual do modelo em imagem ou PDF.",
    emptyLabel: "Categoria preparada para próxima fase",
    id: "composite",
  ...
  {
    description: "Registros de trabalhos já realizados.",
    emptyLabel: "Categoria preparada para próxima fase",
    id: "work-videos",
    placeholder: "Vídeo",
    title: "Work videos"
  ...
  {
    description: "Apresentações em vídeo para casting.",
    emptyLabel: "Categoria preparada para próxima fase",
    id: "video-casting",
    placeholder: "Vídeo",
    title: "Video casting"
  ...
  {
    description: "Materiais recebidos de agências parceiras.",
    emptyLabel: "Categoria preparada para próxima fase",
    id: "mother-agency",
    placeholder: "Arquivo privado",
  ...
  {
    description: "Pacotes e seleções preparados para clientes.",
    emptyLabel: "Categoria preparada para próxima fase",
    id: "client-materials",
    placeholder: "Arquivo privado",
  ...
  return (
    <section className="media-section">
      <div className="media-section-header">
        ...
        {showUpload ? (
          <MediaUploadArea category={category} modelId={modelId} />
        ) : null}
      </div>
...
function MediaTab({ profile }: { profile: ModelProfile }) {
  ...
  const itemsByType = new Map<ModelMedia["media_type"], ModelMedia[]>();

  for (const item of media) {
    const items = itemsByType.get(item.media_type) ?? [];
    items.push(item);
    itemsByType.set(item.media_type, items);
  }
  ...
  return (
    <div className="stack">
      ...
      <div className="media-hub">
        {mediaCategories.map((category) => (
          <MediaCategorySection
            category={category}
            items={
              category.mediaType ? itemsByType.get(category.mediaType) ?? [] : []
            }
            key={category.id}
            modelId={model.id}
            showUpload
      ...
      <details className="media-edit-panel">
        <summary>Editar galeria</summary>
        <p>
          As ações abaixo usam signed URL curta quando necessário. A exclusão
          remove o arquivo e o registro.
        </p>
        <div className="media-hub media-hub-editing">
          {mediaCategories.map((category) => (
            <MediaCategorySection
              category={category}
              isEditing
              items={
                category.mediaType
                  ? itemsByType.get(category.mediaType) ?? []
                  : []
              }
              key={category.id}
              modelId={model.id}
